...
function get_replenish_list()
{
    ...
    $where .= (!empty($filter['order_sn'])) ? " AND order_sn LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$filter['order_sn']) . "%' " : '';
    ...
    if (!empty($filter['keyword']))
    {
        ...
        {
            $where .= " AND order_sn LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$filter['keyword']). "%' ";
        }
    }
