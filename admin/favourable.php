...
elseif ($_REQUEST['act'] == 'search')
{
    ...
    {
        $sql = "SELECT cat_id AS id, cat_name AS name FROM " . $ecs->table('category') .
            " WHERE cat_name LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$filter->keyword) . "%' LIMIT 50";
        $arr = $db->getAll($sql);
    }
    elseif ($filter->act_range == FAR_BRAND)
    {
        $sql = "SELECT brand_id AS id, brand_name AS name FROM " . $ecs->table('brand') .
            " WHERE brand_name LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$filter->keyword) . "%' LIMIT 50";
        $arr = $db->getAll($sql);
    }
    else
    {
        $sql = "SELECT goods_id AS id, goods_name AS name FROM " . $ecs->table('goods') .
            " WHERE goods_name LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$filter->keyword) . "%'" .
            " OR goods_sn LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$filter->keyword) . "%' LIMIT 50";
        $arr = $db->getAll($sql);
    }
    if (empty($arr))
    {
...
function favourable_list()
{
    ...
    if ($result === false)
    {
        ...
        {
            $where .= " AND act_name LIKE '%" .mysqli_real_escape_string($GLOBALS['db']->link_id,$filter['keyword']) . "%'";
        }
        if ($filter['is_going'])
        {
            $now = gmtime();
