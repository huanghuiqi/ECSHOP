...
elseif ($_REQUEST['act'] == 'search_users')
{
    ...
    if ($id_name != '')
    {
        $sql = "SELECT user_id, user_name FROM " . $GLOBALS['ecs']->table('users') .
                " WHERE user_id LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$id_name) . "%'" .
                " OR user_name LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$id_name) . "%'" .
                " LIMIT 20";
...
elseif ($_REQUEST['act'] == 'search_goods')
{
    ...
    if ($keyword != '')
    {
        $sql = "SELECT goods_id, goods_name, goods_sn FROM " . $GLOBALS['ecs']->table('goods') .
                ...
                " AND is_on_sale = 1" .
                " AND is_alone_sale = 1" .
                " AND (goods_id LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$keyword) . "%'" .
                " OR goods_name LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$keyword) . "%'" .
                " OR goods_sn LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$keyword) . "%')" .
                " LIMIT 20";
        $res = $GLOBALS['db']->query($sql);
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['order_sn'])
        {
            $where .= " AND o.order_sn LIKE '%" .mysqli_real_escape_string($GLOBALS['db']->link_id,$filter['order_sn']) . "%'";
        }
        if ($filter['consignee'])
        {
            $where .= " AND o.consignee LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$filter['consignee']) . "%'";
        }
        if ($filter['email'])
        {
            $where .= " AND o.email LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$filter['email']) . "%'";
        }
        if ($filter['address'])
        {
            $where .= " AND o.address LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$filter['address']) . "%'";
        }
        if ($filter['zipcode'])
        {
            $where .= " AND o.zipcode LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$filter['zipcode']) . "%'";
        }
        if ($filter['tel'])
        {
            $where .= " AND o.tel LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$filter['tel']) . "%'";
        }
        if ($filter['mobile'])
        {
            $where .= " AND o.mobile LIKE '%" .mysqli_real_escape_string($GLOBALS['db']->link_id,$filter['mobile']) . "%'";
        }
        if ($filter['country'])
        {
        ...
        if ($filter['user_name'])
        {
            $where .= " AND u.user_name LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$filter['user_name']) . "%'";
        }
...
function delivery_list()
{
    ...
    if ($result === false)
    {
        ...
        if ($filter['order_sn'])
        {
            $where .= " AND order_sn LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$filter['order_sn']) . "%'";
        }
        if ($filter['consignee'])
        {
            $where .= " AND consignee LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$filter['consignee']) . "%'";
        }
        if ($filter['status'] >= 0)
        {
            $where .= " AND status = '" . mysqli_real_escape_string($GLOBALS['db']->link_id,$filter['status']) . "'";
        }
        if ($filter['delivery_sn'])
        {
            $where .= " AND delivery_sn LIKE '%" . mysqli_real_escape_string($GLOBALS['db']->link_id,$filter['delivery_sn']) . "%'";
        }
