...
{
    $tables = $db->GetCol("SHOW TABLES LIKE '" . mysqli_real_escape_string($GLOBALS['db']->link_id,$ecs->prefix) . "%'");
...
if ($_REQUEST['act'] == 'dumpsql')
{
    ...
    switch ($type)
    {
        case 'full':
            ...
            $temp = $db->GetCol("SHOW TABLES LIKE '" . mysqli_real_escape_string($GLOBALS['db']->link_id,$ecs->prefix) . "%'");
...
if ($_REQUEST['act'] == 'optimize')
{
    ...
    $db_ver = $db_ver_arr;
    $ret = $db ->query("SHOW TABLE STATUS LIKE '" . mysqli_real_escape_string($GLOBALS['db']->link_id,$ecs->prefix) . "%'");
